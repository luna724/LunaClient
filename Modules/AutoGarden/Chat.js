...
register("chat",
  (chat, event) => {
    ...
    if (chat.startsWith("[LC-AutoMove]: Changed direction to")) {
      ...
      send(
        uncolored + `Changed direction to ${direction}`
      );
...
export function startTextConverterInAutoGarden() {
  //textConverterInAutoGarden = textConverterInAutoGarden.register();
}

export function stopTextConverterInAutoGarden() {
  //textConverterInAutoGarden = textConverterInAutoGarden.unregister();
}